...
class PyMBSD(object):
    ...
    def __init__(self,
        ...
        print '------------------------------\
               ...
               \n\
               \n det(ResponsePlasticBeta): \t {:0.2E}\
               \n det(ResponsePlasticGamma): \t {:0.2E}\
               \n det(ResponseLaBr3Beta): \t {:0.2E}\
               \n det(ResponseLaBr3Gamma): \t {:0.2E}\
               \n\
               \n------------------------------\
               '.format(np.round(np.linalg.det(self.ResponseBetaPlastic[0])),
                        np.round(np.linalg.det(self.ResponseGammaPlastic[0])),
                        np.round(np.linalg.det(self.ResponseBetaLaBr3[0])),
                        np.round(np.linalg.det(self.ResponseGammaLaBr3[0])))
    ...
    def loadDoseCoeff(self, fName=''):
        '''
        Function to load the dose coefficients
        '''

        # Import operational dose coefficients from files
        df_AirKerma_Photon = pandas.read_excel(fName, sheet_name = 'Photon (Ka)')
        df_Photon_Hp10 = pandas.read_excel(fName, sheet_name = 'Photon (Ka-to-Hp(10))')
        df_Photon_D3 = pandas.read_excel(fName, sheet_name = 'Photon (phi-to-D(3))')
        df_Photon_Hp0p07 = pandas.read_excel(fName, sheet_name = 'Photon (Ka-to-Hp(0.07))')
        ...
        self.coeffGammaWB[0] *= np.array([logInterpCoeff(self.ResponseBetaPlastic[1][0], 
                                                         df_Photon_Hp10['Energy (MeV)'].values*1E3, 
                                                         df_Photon_Hp10['Hp(10,0)/Ka (Sv/Gy)'].values),
        ...
        self.coeffGammaSkin[0] *= np.array([logInterpCoeff(self.ResponseBetaPlastic[1][0], 
                                                           df_Photon_Hp0p07['Energy (MeV)'].values*1E3, 
                                                           df_Photon_Hp0p07['Hp(0.07,0)/Ka (Sv/Gy)'].values),
                                            self.ResponseBetaPlastic[1][0]])[0]

        # Load the dose coefficients into self object
        # NOTE: Energy scaling from MeV to keV
        self.coeffBetaWB = np.array([logInterpCoeff(self.ResponseBetaPlastic[1][0], 
                                                    df_Electron_Hp10['Energy (MeV)'].values*1E3, 
                                                    df_Electron_Hp10['Hp(10,0)/phi (pSv cm2)'].values*coeffScalingFactor),
                                     self.ResponseBetaPlastic[1][0]])
        ...
        self.coeffBetaSkin = np.array([logInterpCoeff(self.ResponseBetaPlastic[1][0], 
                                                      df_Electron_Hp0p07['Energy (MeV)'].values*1E3, 
                                                      df_Electron_Hp0p07['Hp(0.07,0)/phi (nSv cm2)'].values),
                                       self.ResponseBetaPlastic[1][0]])
